...
/**
 * Director-only: create or lift an umpire suspension (mid-tournament OK).
 * Same shape as player suspensions — tournament and/or dates + note.
 */
export default function SuspendUmpire({
  umpire,
  tournaments = [],
  suspensions = [],
  ...
  const name =
    umpire.displayName ||
    [umpire.lastName, umpire.firstName].filter(Boolean).join(", ") ||
    umpire.name ||
    "Umpire";
    ...
    <>
      ...
      {open && (
        <Modal
          ...
          subtitle="They stay on the umpire roster. Assignments show them as suspended so the field desk does not miss it."
            ...
            <div className="space-y-2">
              ...
              <label className="block">
                ...
                <textarea
                  ...
                  placeholder="e.g. No-show Saturday — rest of weekend"
                  disabled={busy}
                />
              </label>
            </div>
