...
/**
 * Official roster / waiver line: legal first + last, then (preferred)
 * when preferred is a real nickname. "James Willcox (JD)".
 */
export function waiverFormName({
  legalFirstName,
  legalLastName,
  preferredName,
  name,
  ...
  const legal =
    composeLegalName({ legalFirstName, legalLastName }) ||
    String(name ?? "").trim();
  const nick = String(preferredName ?? "").trim();
  if (!nick || !legal) return legal || nick || "";
  if (preferredIsFullLegal({ preferredName: nick, legalFirstName, legalLastName, name })) {
    return legal;
  }
  const first = String(legalFirstName ?? "").trim();
  if (first && nick.toLowerCase() === first.toLowerCase()) return legal;
  if (legal.toLowerCase() === nick.toLowerCase()) return legal;
  if (legal.toLowerCase().startsWith(`${nick.toLowerCase()} `)) return legal;
  return `${legal} (${nick})`;
}
