...
//
// Same player on two teams in one tournament is allowed (managers may add),
// but both sides get a director "Check" flag. Same-gender doubles are called
// out more strongly in the warning text (AFA policy concern).
...
/**
 * Probe for dual-roster. Always allows the add (ok: true) with warnings.
 * Same-gender conflicts are marked so UI can stress them.
 *
 * @returns {Promise<{
 *   ok: true,
 *   warnings: string[],
...
export async function checkPlayerDualRoster(supabase, args) {
  const otherTeams = await findPlayerOtherTeams(supabase, args);
  if (otherTeams.length === 0) {
    return { ok: true, warnings: [], otherTeams: [], hasSameGender: false };
  }

  const hasSameGender = otherTeams.some((o) => o.sameGender);
  const warnings = otherTeams.map((o) => {
    const scope = o.sameGender
      ? "same gender in this tournament"
      : "another division in this tournament";
    return `${args.name?.trim() || "This player"} is already on ${o.teamName} (${scope}).`;
  });

  if (hasSameGender) {
    warnings.push(
      "Same-gender dual roster is usually not allowed under AFA rules — the director will see a Check flag on both teams."
    );
  } else {
    warnings.push(
      "Both teams will show a Check flag so the director can review."
    );
  }

  return { ok: true, warnings, otherTeams, hasSameGender };
}
...
/**
 * For a full roster, find members who also appear on other teams this event.
 *
...
export async function dualRosterCheckForRoster(supabase, {
  ...
  const others = (regs ?? []).filter((r) => r.id !== registrationId);
  ...
  for (const m of active) {
    ...
    for (const o of allMembers ?? []) {
      if (
        samePerson(o, { name, birthDate, playerId })
      ) {
