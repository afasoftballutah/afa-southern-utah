...
export async function checkPlayerDualRoster(supabase, args) {
  ...
  const warnings = otherTeams.map(
    (o) =>
      `${args.name?.trim() || "This player"} is already on ${o.teamName} (same gender this tournament).`
  );
  warnings.push(
    "Same-gender dual roster is not allowed under AFA rules — both teams will show a Check flag for the director."
  );
