...
/**
 * Does this roster clear the limits of a named class?
 *
 * Returns the ratings that are over, so the UI can point at the actual people
 * rather than describe the problem in a sentence. JD, 2026-07-27: a paragraph
 * of explanation is "claude nonsense speak" — show the roster and mark it.
 */
export function checkEligibility(players, className) {
  const n = countRatings(players);
  const rule = RULES[className];
  if (!rule) return { ok: true, over: [], limit: null };
  if (rule.test(n)) return { ok: true, over: [], limit: rule.describe() };

  const over = [];
  if (n.A > 0) over.push("A");
  if (n.B > 0) over.push("B");
  if (className === "Rec") {
    for (const r of RATINGS) if (n[r] > 0 && !over.includes(r)) over.push(r);
  } else if (className === "E") {
    if (n.C > 1) over.push("C");
    else if (n.C === 1 && n.D > 1) over.push("D");
    else if (n.C === 0 && n.D > 3) over.push("D");
  } else if (className === "D") {
    if (n.C > 3) over.push("C");
  }
  return { ok: false, over, limit: rule.describe() };
}

function countRatings(players) {
  const n = Object.fromEntries(RATINGS.map((r) => [r, 0]));
  for (const p of players ?? []) if (isRanked(p.rating)) n[p.rating] += 1;
  return n;
}
...
export function suggestClass(players, classes, offeredClassIds = null) {
  ...
  const n = countRatings(roster);
  const unranked = roster.filter((p) => !isRanked(p.rating)).length;
...
/**
 * Does this roster have enough men and women for the division?
 *
 * JD, 2026-07-27: "more than the required is fine, but less than is a flag
 * that a team cant compete."
 *
 * A FLAG, never a block. A roster is a snapshot — people get added the week
 * of, and a director knows things the list does not say. Unrecorded gender is
 * called out separately from a genuine shortfall, because "we do not know" and
 * "they are short" are different problems with different fixes.
 */
export function checkRoster(players, { minMen, minWomen } = {}) {
  if (minMen == null && minWomen == null) {
    return { ok: true, men: 0, women: 0, unknown: 0, short: [] };
  }
  const roster = players ?? [];
  const men = roster.filter((p) => p.gender === "M").length;
  const women = roster.filter((p) => p.gender === "F").length;
  const unknown = roster.filter((p) => p.gender !== "M" && p.gender !== "F").length;

  const short = [];
  if (minMen != null && men < minMen) short.push({ what: "men", have: men, need: minMen });
  if (minWomen != null && women < minWomen) {
    short.push({ what: "women", have: women, need: minWomen });
  }

  return { ok: short.length === 0, men, women, unknown, minMen, minWomen, short };
}
...
const CLASS_TOKEN = /^(open|d|e|rec)$/i;

/**
 * The class letter a director sees on a registration.
 * Prefer class_id, then the division's class, then a stored class name
 * that is actually Rec/E/D/Open — never a division label like "Men's D".
 */
export function enteredClassName(reg, classes = []) {
  const byId = (id) =>
    id ? (classes.find((c) => c.id === id)?.name ?? null) : null;
  const named = byId(reg?.class_id);
  if (named) return named;
  const fromDivision = byId(reg?.divisions?.class_id);
  if (fromDivision) return fromDivision;

  const known = new Map(
    (classes ?? []).map((c) => [String(c.name).toLowerCase(), c.name])
  );
  const raw = String(reg?.class ?? "").trim();
  if (raw && known.has(raw.toLowerCase())) return known.get(raw.toLowerCase());

  const label = String(
    reg?.divisions?.display_name ?? reg?.divisions?.name ?? raw ?? ""
  ).trim();
  const token = label.split(/[\s/·.\-]+/).filter(Boolean).pop();
  if (token && CLASS_TOKEN.test(token) && known.has(token.toLowerCase())) {
    return known.get(token.toLowerCase());
  }
  return null;
}
