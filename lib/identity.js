 ...
 *
 * Identity is normalized LEGAL name + birth date (preferred is display only).
 * Without a birth date there is no safe key.
 *
...
export async function resolvePlayer(
  ...
  const legal = [legalFirstName, legalLastName]
    .map((s) => String(s ?? "").trim())
    .filter(Boolean)
    .join(" ");
  const identityName = legal || String(name ?? "").trim();
  // Roster/display name: preferred, else first name only (not full legal).
  const display =
    composeDisplayName({
      preferredName,
      legalFirstName,
  ...
  const normalized = normalizeName(identityName);
  ...
  if (existing) {
    ...
    // Soft-refresh contact / name fields when we learn more
    const patch = {};
    if (legalFirstName) patch.legal_first_name = String(legalFirstName).trim();
    if (legalLastName) patch.legal_last_name = String(legalLastName).trim();
    if (preferredName) patch.preferred_name = String(preferredName).trim();
    if (email) patch.email = String(email).trim();
    if (display) patch.full_name = display;
  ...
  const { data: created, error } = await supabase
    ...
    .insert({
      ...
      legal_first_name: legalFirstName
        ? String(legalFirstName).trim()
        : null,
      legal_last_name: legalLastName ? String(legalLastName).trim() : null,
      preferred_name: preferredName ? String(preferredName).trim() : null,
