...
/** Color key + labels for a single division seat (Women's D, Coed, …). */
export function seatFromDivision(d) {
  if (!d) return null;
  const key = divisionGenderKey(d);
  const row = GENDER_ROWS.find((r) => r.key === key) ?? {
    key: "coed",
    label: "Coed",
  };
  const genderOnly = isBareGenderDivision(d, row.label);
  const level = divisionLevelLabel(d);
  return {
    genderKey: key,
    genderLabel: row.label,
    levelLabel: genderOnly ? "" : level,
    seatLabel: genderOnly ? row.label : `${row.label} ${level}`,
  };
}
