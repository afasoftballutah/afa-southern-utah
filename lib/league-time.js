...
/**
 * Empty game-time picker. Minutes are always :00 so a phone clock is not
 * stuck on "now" (10:37). Hour is 8:00, a typical first game; the director
 * rolls hours, not minutes.
 */
export function defaultGameWhenInput(playDay) {
  if (playDay) return "08:00";
  const p = leagueParts(new Date());
  return p ? `${p.year}-${p.month}-${p.day}T08:00` : "";
}
...
export function parseGameWhenInput(value, playDay) {
  ...
  if (!raw) return null;
  if (playDay) {
    // A leftover datetime-local, or a phone `time` value with seconds.
    if (raw.includes("T")) return parseLeagueInputValue(raw);
    const t = timeInputValue(raw) || raw.slice(0, 5);
    if (!/^\d{2}:\d{2}$/.test(t)) return null;
    return parseLeagueInputValue(`${String(playDay).slice(0, 10)}T${t}`);
  }
  return parseLeagueInputValue(raw);
}
