...
export function championOf(games) {
  const game = championshipGameOf(games);
  return game ? winnerOf(game) : null;
}

/**
 * The championship game itself — the final decided game whose winner feeds
 * nothing (if-game moot rules applied). Null while the bracket is open or
 * ambiguous. Used on the home Recent Results cards.
 */
export function championshipGameOf(games) {
