...
/**
 * @param {string[]} teamNames - in seed order (rank 1 first)
 ...
 *   team1/team2 are either { type: 'team', name } | { type: 'bye' } |
 *   { type: 'ref', side, round, slot, result: 'winner'|'loser' }
 */
export function buildBracketStructure(teamNames, format = "double_elim") {
  ...
  for (let round = 1; round <= k; round++) {
    ...
    for (let slot = 0; slot < count; slot++) {
      ...
      if (round === 1) {
        ...
        team1 = t1 ? { type: "team", name: t1 } : { type: "bye" };
        team2 = t2 ? { type: "team", name: t2 } : { type: "bye" };
      } else {
  ...
  // ---- Losers bracket ----
  const totalLosersRounds = format === "double_elim_consolation" ? 2 * (k - 1) : 2 * (k - 1);
  // NOTE: the "double_elim_consolation" (dropdown) variant is not a distinct
  // topology here — see README "Interpretation call" for why. Same shape as
  // standard double elim for now; format is stored for future use.
  for (let i = 1; i <= k - 1; i++) {
