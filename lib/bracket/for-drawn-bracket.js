...
const SIDE_LABEL = { winners: "Winners", losers: "Losers", final: "Final" };

/**
 * Schedule / calendar caption for one game. Paper G#s (unique `round`)
 * are "Game 4", never "Winners R4" — a 3GG remapped after Generate still
 * has a `brackets` row, so that row is not the signal.
 */
export function scheduleSlotLabel(games, game) {
  if (game?.round == null) return null;
  if (usesPaperGameNumbers(games)) return `Game ${game.round}`;
  return `${SIDE_LABEL[game.bracket_side] ?? game.bracket_side} R${game.round}`;
}
