...
/** Signature on top, validation stamp tucked under it. */
async function drawSignedBlock(doc, page, {
  x, y, maxWidth, maxHeight, signaturePng, signedAt, signedPlace, font, black,
}) {
  const stamp = signedAt
    ? formatSignRecord({ signedAt, signedPlace })
    : "";
  const stampSpace = stamp ? STAMP_H : 0;
  await drawSignatureOrPlaceholder(doc, page, {
    x,
    y: y + stampSpace,
    maxWidth,
    maxHeight: Math.max(10, maxHeight - stampSpace),
    signaturePng,
    font,
    black,
  });
...
export async function buildWaiverPdf({
  ...
  for (const p of players) {
    ...
    const stampH = p.signedAt ? STAMP_H : 0;
    const rowH = Math.max(26, lineCount * lineH + 6 + stampH);
    const topY = y;
