...
const waiting = rgb(0.55, 0.15, 0.15);

/**
 * Draw a signature (or “awaiting”) on the same baseline as the name.
 * If signed, tuck the time/place stamp immediately under the ink.
 */
async function drawSignedBlock(doc, page, {
  x,
  baselineY,
  maxWidth,
  signaturePng,
  ...
  const stamp = signedAt
    ? formatSignRecord({ signedAt, signedPlace })
    : "";
  ...
  try {
    ...
    const scale = Math.min(maxWidth / pngImage.width, SIG_H / pngImage.height);
    const w = pngImage.width * scale;
    const h = pngImage.height * scale;
    // Sit the ink on the name line (image bottom just under the baseline).
    page.drawImage(pngImage, { x, y: baselineY - 2, width: w, height: h });
  } catch {
  ...
  if (stamp) {
    // Sit on the bottom of the ink — a little overlap is fine.
    page.drawText(stamp, {
      x,
      y: baselineY - 3,
      size: 5.5,
...
export async function buildWaiverPdf({
  ...
  await drawSignedBlock(doc, page, {
    x: left + 70,
    baselineY: y - sigHeight / 2,
    maxWidth: 140,
    signaturePng: registration.managerSignaturePng,
    signedAt: registration.managerSignedAt,
    signedPlace: registration.managerSignedPlace,
    font,
    black,
  });
  y -= sigHeight + 14;

  const players = rosterMembers.filter((r) => r.role !== "coach");
  ...
  for (const p of players) {
    ...
    const stampH = p.signaturePng && p.signedAt ? 4 : 0;
    const rowH = Math.max(lineCount * lineH, 11) + stampH + 4;
    const topY = y;
    ...
    page.drawText(formatDob(p.birthDate), {
      x: colX.birth,
      y: topY,
      size: 8,
      font,
