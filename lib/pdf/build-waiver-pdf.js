...
/** Drop trailing commas/spaces from Google-style formatted addresses. */
...
export async function buildWaiverPdf({ registration, rosterMembers }) {
  ...
  y -= 14;
  page.drawText(
    `Address: ${cleanAddress(m.address)}    City: ${m.city || ""}    State: ${m.state || ""}    Zip: ${m.zip || ""}`,
    { x: left, y, size: 9, font, color: black }
  );
  y -= 6;
  ...
  for (const p of players) {
    ...
    page.drawText((p.name || "").slice(0, 28), {
      x: colX.name,
      y: y + 6,
      size: 8,
      font,
    ...
    page.drawText((p.email || "").slice(0, 28), {
      x: colX.email,
      y: y + 6,
      size: 7,
      font,
    ...
    page.drawText(cleanAddress(p.address).slice(0, 36), {
      x: colX.address,
      y: y + 6,
      size: 7,
      font,
