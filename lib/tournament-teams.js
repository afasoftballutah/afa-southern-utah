...
export async function listTournamentTeams(tournament) {
  ...
  const { data, error } = await supabase
    .from("registrations")
    .select(
      "team_name, manager_name, manager_email, division_id, status, divisions(id, name, display_name, gender)"
    )
  ...
  for (const row of data ?? []) {
    ...
    const managerEmail = String(row.manager_email ?? "").trim().toLowerCase();
    list.push({
      name,
