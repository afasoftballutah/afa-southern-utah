 ...
 *
 * “Waiting to sign” = people on the Players list with at least one unsigned
 * active appearance — same set as the “Waiver missing” filter. Not a sum of
 * roster slots from registration_signing_progress (that was 18 vs 11 on file
 * and did not match the directory).
 */
export async function getDirectorCounts() {
  const supabase = getServiceClient();
  const [players, teams, registrations, tournaments, members] =
    await Promise.all([
      supabase
        .from("players")
      ...
      supabase
        .from("roster_members")
        .select("player_id, signed_at, removed_at")
  ...
  // Unique people still missing a signature on at least one active roster row.
  const unsignedPeople = new Set(
    (members.data ?? []).map((m) => m.player_id).filter(Boolean)
  ).size;
  ...
  return {
    ...
    outstandingSignatures: unsignedPeople,
...
export async function listPeople() {
  ...
  const [
    ...
    supabase
      .from("roster_members")
      .select(
        "id, player_id, registration_id, name, role, signed_at, removed_at, email, phone, address, legal_first_name, legal_last_name, preferred_name"
      ),
    supabase
      .from("registrations")
  ...
  return {
    ...
    players: (players ?? [])
      ...
      .map((p) => {
        ...
        const email =
          p.email ||
          appearances.find((a) => a.email)?.email ||
          null;
        return {
          ...p,
