...
export async function getDirectorCounts() {
  ...
  const [players, teams, registrations, tournaments, members] =
    await Promise.all([
      ...
      supabase
        .from("roster_members")
        .select("id, player_id, signed_at, removed_at")
        .is("removed_at", null),
    ]);
  ...
  // One row per unlinked roster seat — same as the Players list provisional rows.
  const provisional = active.filter((m) => !m.player_id).length;
  // Unique confirmed people with any unsigned seat, plus every unlinked unsigned seat.
  const unsignedConfirmed = new Set(
    active
      .filter((m) => m.player_id && !m.signed_at)
      .map((m) => m.player_id)
  ).size;
  const unsignedProvisional = active.filter(
    (m) => !m.player_id && !m.signed_at
  ).length;
  ...
  return {
    ...
    outstandingSignatures: unsignedConfirmed + unsignedProvisional,
...
export async function listPeople() {
  ...
  const [
    ...
    supabase
      .from("roster_members")
      .select(
        "id, player_id, registration_id, name, role, signed_at, removed_at, email, phone, address, legal_first_name, legal_last_name, preferred_name, gender, birth_date"
      ),
    supabase
      .from("registrations")
  ...
  const provisional = (members ?? [])
    ...
    .map((m) => {
      ...
      return {
        ...
        rating: null,
        email: m.email || appearance.managerEmail || null,
