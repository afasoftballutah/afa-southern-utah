...
import { championOf, mootIfRounds } from "@/lib/bracket/if-game";
import { scheduleSlotLabel } from "@/lib/bracket/for-drawn-bracket";
...
// scoped to this region. Colorado / Arizona / Nevada / Northern Utah are
// mostly schedule-only (Fredonia AZ is still registerable via explicit allow).
...
// Regions on the public Tournaments page (home base first).
...
export const REGION_LABEL = {
  ...
  arizona: "Arizona",
  nevada: "Nevada",
  // Legacy short codes / pre-split series — mapped by canonicalRegion
  co: "Colorado",
  az: "Arizona",
  nv: "Nevada",
  series: "Colorado",
...
/**
 * Public region for grouping/labels. Maps legacy short codes (`co`/`az`/`nv`),
 * `series`, and a few geographic overrides to colorado|arizona|nevada|….
 * Operational scope (register / brackets) still keys off the raw
 * southern_utah column where needed.
 */
export function canonicalRegion(t) {
  const raw = String(t?.region ?? "southern_utah");
  const slug = String(t?.slug ?? "");
  const venue = `${t?.venue_name ?? ""} ${t?.venue_address ?? ""} ${t?.name ?? ""}`.toLowerCase();

  // Explicit overrides
  if (slug === "2026-coed-fredonia") return "arizona";
  if (slug === "2026-afa-world-series") return "nevada";
  if (slug === "2026-best-of-the-west") return "colorado";

  // Full names (current) and short codes (legacy rows / mid-migrate)
  if (raw === "colorado" || raw === "co") return "colorado";
  if (raw === "arizona" || raw === "az") return "arizona";
  if (raw === "nevada" || raw === "nv") return "nevada";

  if (raw === "southern_utah" || raw === "northern_utah") {
    // Wendover / Mesquite sit in Nevada even if older rows said UT
    if (venue.includes("wendover") || venue.includes("mesquite")) return "nevada";
    return raw;
  }

  // Legacy AFA Tournament Series → geography
  if (raw === "series") {
    if (
      venue.includes("montrose") ||
      venue.includes("delta") ||
      venue.includes(", co") ||
      venue.includes("colorado")
    ) {
      return "colorado";
    }
    if (venue.includes("wendover") || venue.includes("mesquite") || venue.includes(", nv")) {
      return "nevada";
    }
    if (venue.includes("fredonia") || venue.includes(", az") || venue.includes("arizona")) {
      return "arizona";
    }
    // Roosevelt UT series stops, etc.
    return "northern_utah";
  }

  return "southern_utah";
}
...
export async function getSeasonListByRegion() {
  ...
  const rows = (data ?? []).map((t) => ({
    ...t,
    // Keep raw region; expose canonical for grouping
    region: canonicalRegion(t),
    region_raw: t.region,
  }));
...
// Real per-event poster (not a shared season/region schedule flyer).
// Public site + registration only surface tournaments that pass this —
// the rest stay in the DB and appear greyed on the list until a flyer ships.
const SCHEDULE_POSTER_MARKERS = [
  "2026-schedule",
  "northern-utah-schedule",
  "schedule.jpg",
  "schedule.png",
];
export function isRealPoster(t) {
  const url = String(t?.poster_url ?? "").trim();
  if (!url) return false;
  const lower = url.toLowerCase();
  return !SCHEDULE_POSTER_MARKERS.some((m) => lower.includes(m));
}

/** Region schedule flyer shown under unpublished (no event poster) events. */
export function schedulePosterForRegion(region) {
  if (region === "northern_utah") return "/posters/2026-northern-utah-schedule.jpg";
  // Other regions: no local multi-event schedule asset yet
  return null;
}
...
export async function getTournamentSchedule(slug) {
  ...
  for (const division of tournament.divisions ?? []) {
    ...
    // `round` is a paper G# when every game has its own number (hand-built
    // or remapped 3GG). A `brackets` row is NOT the signal — Generate then
    // remap still has one, and labelling G4 "Winners R4" is what the
    // calendar was doing.
    const divisionGames = division.games ?? [];
    ...
    const moot = mootIfRounds(divisionGames);
    ...
    for (const g of divisionGames) {
      ...
      rows.push({
        ...
        isFinal: g.status === "final",
        divisionName,
        label: scheduleSlotLabel(divisionGames, g),
      });
    }
  }
  return rows;
...
export async function getRecentScores(limit = 5, divisionIds = null) {
  // null/undefined = league-wide. [] = this tournament has no divisions →
  // empty, not "all games" (Halloween showed Heat Stroker results that way).
  ...
  return [
    ...
    ...(bracket.data ?? []).map((g) => shape(g, g.round != null ? `G${g.round}` : null)),
  ]
    .sort(
      (a, b) =>
...
export async function getUpcomingGames(limit = 8, divisionIds = null) {
  // Same contract as getRecentScores: [] is "no divisions here", not global.
  if (Array.isArray(divisionIds) && divisionIds.length === 0) return [];
  ...
  return [
    ...
    ...(bracket.data ?? [])
      .filter(stillToPlay)
      .map((g) => shape(g, g.round != null ? `G${g.round}` : null)),
  ]
...
export async function getTeamSummaries(tournament) {
  ...
  const consider = (team, opponent, g, stageLabel, divisionId, isBracket) => {
    if (!real(team)) return;
    const t = touch(team);
    if (divisionId && !t.divisionIds.includes(divisionId)) {
      t.divisionIds.push(divisionId);
    }
    ...
    if (!t.next || when < t.next.when) {
      t.next = {
        ...
        label: isBracket ? (g.round != null ? `G${g.round}` : null) : `Pool ${g.pool}`,
        divisionId,
...
/**
 * Finished Southern Utah tournaments (newest first), with games for
 * championship finals. Used by home Recent Results dropdown.
 */
export async function getRecentCompletedTournaments(limit = 8) {
  const supabase = getPublicClient();
  const { data, error } = await supabase
    .from("tournaments")
    ...
    .limit(12);
  if (error) throw error;
  const finished = (data ?? []).filter((t) => isTournamentFinished(t));
  return finished.slice(0, Math.max(1, limit));
}

export async function getLastCompletedTournamentResults() {
  const list = await getRecentCompletedTournaments(1);
  return list[0] ?? null;
}
