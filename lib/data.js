...
export async function getTeamStatus(tournamentId) {
  ...
  const { data, error } = await supabase
    .from("team_status")
    .select("team_name, state, last_game_label, last_game_at")
...
export async function getRecentScores(limit = 5) {
  ...
  const [pool, bracket] = await Promise.all([
    supabase
      .from("pool_games")
      .select("id, pool, field, scheduled_time, team1_name, team2_name, team1_score, team2_score, divisions(name, display_name, tournaments(name, slug))")
      .eq("status", "final")
      .order("scheduled_time", { ascending: false })
      .limit(limit),
    supabase
      .from("games")
      .select("id, round, field, scheduled_time, team1_name, team2_name, team1_score, team2_score, divisions(name, display_name, tournaments(name, slug))")
      .eq("status", "final")
      .order("scheduled_time", { ascending: false })
      .limit(limit),
  ]);
