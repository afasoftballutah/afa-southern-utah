...
// Lightweight read for the .ics route — only the fields an all-day calendar
// event needs, no divisions/placements/brackets/games nesting.
export async function getTournamentBasicsBySlug(slug) {
  const supabase = getPublicClient();
  const { data, error } = await supabase
    .from("tournaments")
    .select("slug, name, start_date, end_date, venue_name, venue_address")
    .eq("slug", slug)
    .maybeSingle();
  if (error) throw error;
  return data;
