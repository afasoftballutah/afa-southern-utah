...
/**
 * Directory label: "Anderson, Kaydee [Kay]" when preferred differs from legal.
 * Prefer stored legal_first/last; fall back to splitting full_name.
 */
export function directoryNameLabel({
  ...
  const base = legal ? lastNameFirst(legal) : lastNameFirst(fullName);
  if (!base && !preferred) return "—";
  if (!preferred) return base;
  const p = preferred.toLowerCase();
  // Full legal as preferred (old backfill) or preferred == first name only → no bracket
  if (
    p === legal.toLowerCase() ||
    p === String(fullName ?? "").trim().toLowerCase() ||
    (first && p === first.toLowerCase())
  ) {
    return base;
  }
